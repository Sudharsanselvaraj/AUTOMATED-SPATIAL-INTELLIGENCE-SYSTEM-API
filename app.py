...
def cache_key(data_type, value, analysis_type):
...
def normalise_request(req: LocationRequest):
    ...
    return dt, req.value, lon, lat, lot_ids, extents
...
def search(q: str, limit: int = 100):
    ...
    try:
        ...
        logging.warning(f"/search lot failed: {e}")

    # ADDRESS search (only when query doesn't look like a lot)
    if not _is_lot(q):
        try:
            resp = requests.get(
            ...
            logging.warning(f"/search address failed: {e}")

    logging.info(f"Search '{q}' → {len(results)} results")
    return {"count": len(results), "results": results}


# ─── LOT BOUNDARY PREVIEW ─────────────────────────────────────────────────────
# Called by the frontend Leaflet map to draw the blue polygon on the
# "Your Site Preview" panel (as seen in the reference Image 2).
# Single lot: GET /lot-boundary?lon=114.15&lat=22.28&data_type=LOT
# Multi-lot:  GET /lot-boundary?lon=...&lat=...&data_type=LOT&extents=[{...},{...}]
# Returns merged GeoJSON polygon in WGS84 — always a SINGLE unified shape.
...
def lot_boundary(
    ...
    import json
    from modules.resolver import get_lot_boundary as _glb
    ...
    try:
        gdf = _glb(lon, lat, data_type, ext_list if ext_list else None)
...
# ─── PDF REPORT ───────────────────────────────────────────────────────────────
def generate_pdf_report(data_type, value, lon=None, lat=None, lot_ids=None, extents=None):
    lot_ids = lot_ids or []
    extents = extents or []
    buffer  = BytesIO()
